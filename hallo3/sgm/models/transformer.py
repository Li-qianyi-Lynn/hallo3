...
class ClipProjModel(torch.nn.Module):
    def __init__(
        ...
        self.hidden_size = hidden_size
        self.proj = torch.nn.Linear(
            clip_embeddings_dim, hidden_size
        )
        self.norm = torch.nn.LayerNorm(hidden_size)
...
class BaseTransformerLayer(torch.nn.Module):
    def __init__(
            ...
            face_cross_attn_hidden_size=None,
            use_bias=True,
            use_qkv_bias=False,
        ...
        self.face_input_layernorm = layernorm(hidden_size, eps=layernorm_epsilon)
        self.face_attn =  CrossAttention(
                ...
                output_layer_init_method=output_layer_init_method,
                cross_attn_hidden_size=face_cross_attn_hidden_size,
                bias=use_bias,
    ...
    def __init__(self,
        ...
        def get_layer(layer_id):
            return BaseTransformerLayer(
                ...
                cross_attn_hidden_size=cross_attn_hidden_size,
                face_cross_attn_hidden_size=face_cross_attn_hidden_size,
        ...
        self.face_proj = FaceProjModel(hidden_size=1024)


    def forward(self, input_ids, position_ids, attention_mask, audio_emb, face_emb, *,
        ...
        if self.checkpoint_activations:
            ...
            def custom(start, end, kw_args_index, cross_layer_index):
                def custom_forward(*inputs):
                    ...
                    x_, mask, audio, face_emb, ref_feat = inputs[0], inputs[1], inputs[2], inputs[3], inputs[4]

                    # recover kw_args and output_cross_layer
                    flat_inputs = inputs[5:]
                    ...
                    for i, layer in enumerate(layers_):
                        ...
                        if 'layer_forward' in self.hooks:
                            layer_ret = self.hooks['layer_forward'](
                                x_, mask, audio, face_emb, ref_feat, layer_id=layer.layer_id,
            ...
            output_this_layer = []
            while l < num_layers:
                args = [hidden_states, attention_mask, audio_emb, face_emb, self.layers[l].bank[0]]
            ...
            for i, layer in enumerate(self.layers):
                args = [hidden_states, attention_mask, audio_emb, face_emb]
